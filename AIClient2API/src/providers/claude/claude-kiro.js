        ...
        try {
            ...
            for (const file of files) {
                if (file.endsWith('.json') && file !== targetFileName) {
                    ...
                    if (credentials) {
                        // Only inherit the target's expiry if this sidecar lacks its own —
                        // a sidecar with its own (often later) expiresAt is more accurate (WR-08).
...
async saveCredentialsToFile(filePath, newData) {
    ...
    _generateThinkingPrefix(thinking) {
        ...
        if (type === 'enabled') {
            ...
            return `${KIRO_THINKING.MODE_TAG}enabled</thinking_mode>${KIRO_THINKING.MAX_LEN_TAG}${budget}</max_thinking_length>`;
        }
        ...
        if (type === 'adaptive') {
            ...
            return `${KIRO_THINKING.MODE_TAG}adaptive</thinking_mode>${KIRO_THINKING.EFFORT_TAG}${normalizedEffort}</thinking_effort>`;
        }
