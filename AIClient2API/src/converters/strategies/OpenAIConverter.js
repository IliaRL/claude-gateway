...
export class OpenAIConverter extends BaseConverter {
    ...
    toClaudeStreamChunk(openaiChunk, model, requestId) {
        ...
        if (finishReason) {
            ...
            // Terminal path: finish_reason chunk received. Idempotent — safe if already gone.
            this.cleanupStream(stateKey);
        }
    ...
    /**
     * Release the per-stream state for a given request/chunk id.
     *
     * Callable on ALL terminal paths — finish_reason emission (above), client abort,
     * upstream drop, or error/throw — so a stream that never produced a finish_reason
     * chunk does not leak its streamParams entry until the lazy stale sweep fires.
     *
     * Idempotent and safe on unknown keys (Map.delete never throws and is a no-op
     * when the key is absent), so there is no double-delete hazard.
     *
     * @param {string} requestId - the requestId or chunk.id used as the stream state key
     */
    cleanupStream(requestId) {
        if (!requestId) return;
        this.streamParams.delete(requestId);
    }
    ...
    toClaudeModelList(openaiModels) {
        return {
            models: openaiModels.data.map(m => ({
                // Additive: include id + display_name (Claude Code model discovery format),
                // while preserving the legacy name + description fields for older consumers.
                id: m.id,
    ...
    buildClaudeToolChoice(toolChoice) {
        if (typeof toolChoice === 'string') {
            ...
            const mapped = mapping[toolChoice];
            // Unknown string → omit tool_choice rather than emit invalid { type: undefined }.
    ...
    toGrokRequest(openaiRequest) {
        // Grok payload is built downstream by GrokApiService.buildPayload from the raw structure below.
