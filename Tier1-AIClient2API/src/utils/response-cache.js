...
export function getCacheKey(requestBody, model) {
    ...
    // Stable key: hash of model + messages + tools (system prompt intentionally excluded —
    // it's injected at the adapter layer and not visible in requestBody here)
    const hashInput = JSON.stringify({ model, messages: requestBody.messages, tools: requestBody.tools ?? null });
