...
# PROXY_BASE points directly to Tier1 AIClient2API (:3000).
# LiteLLM (:4000) corrupts Claude SSE streams by re-wrapping each chunk with a new
# message_start — bypassing it fixes the "no response / run stop hooks" failure.
: "${PROXY_BASE:=${AICLIENT_BASE:-http://127.0.0.1:3000}}"
